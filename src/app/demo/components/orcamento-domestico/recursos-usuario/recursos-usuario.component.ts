...
@Component({
  templateUrl: './recursos-usuario.component.html',
  providers: [MessageService, ConfirmationService]
})
export class RecursosUsuarioComponent implements OnInit {
  ...
  buscarRecursosUsuario(event: any): void {
    ...
    this.recursosUsuarioService.buscarTodos(page, size, sort).subscribe({
      next: (data) => {
        this.recursosUsuarioPaginado = data;
      },
      error: (err) => {
  ...
  atualizarRecursosUsuario(): void {
    if (this.recursosUsuario.id) {
      this.loading = true;
      this.recursosUsuarioService.atualizar(this.recursosUsuario.id, this.recursosUsuario).subscribe({
        next: () => {
          this.messageService.add({
            severity: 'success',
            summary: 'Sucesso',
            detail: 'Recurso do usuario atualizado com sucesso'
          });
          this.recursosUsuarioDialog = false;
          this.buscarRecursosUsuario({ first: 0, rows: 10 });
        },
        error: (response) => {
          console.error('Erro ao atualizar recurso do usuario:', response);
          this.messageService.add({
            severity: 'error',
            summary: 'Erro',
  ...
  direcionarParaTelaMetas(recursosUsuario: RecursosUsuario): void {
    ...
    this.router.navigate([`${basePath}/gerenciamento-gasto-metas`], {
      queryParams: { id: recursosUsuario.id, saldo: recursosUsuario.renda }
    });
  }

  salvarOuAtualizar() {
    if (this.recursosUsuario.id != null) {
      this.atualizarRecursosUsuario();
    } else {
      this.salvarRecursosUsuario();
    }
  }
